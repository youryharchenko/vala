...
public Net(int dim, Step[] steps) {
	...
	//foreach (Point p in this.all_points) {
	//    stdout.printf("point (%d, %d) ", p.x, p.y);
	//    p.print_slots();
	//}

	//stdout.printf("Net complete: points: %d, slots:%d\n", this.all_points.length, this.all_slots.length);
...
public Result calculate() {
	//stdout.printf("Net: calculate start\n");
	...
	if (check_win()) {
		...
		return new Result(State.WIN, new_step, "I won!");
	}
	if (check_draw()) {
		stdout.printf("Net: calculate after draw\n");
		return new Result(State.DRAW, new_step, "Draw!");
	}
	Result res = new Result(State.CONTINUE, new_step, "");
	//stdout.printf("Net: calculate end - result: %s\n", res.to_string());
	return res;
}
...
private bool check_win() {
	//stdout.printf("Net: check win start\n");
...
/*
   private Point[] remove_point(Point[] points, int n) {
    int l = points.length;
    for(int i = n; i < l - 1; i++) points[i] = points[i + 1];
    points.resize(l - 1);
    return points;
   }

   private Slot[] remove_slot(Slot[] slots, int n) {
    int l = slots.length;
    for(int i = n; i < l - 1; i++) slots[i] = slots[i + 1];
    slots.resize(l - 1);
    return slots;
   }

   private Point[] append_point(Point[] points, Point p) {
    int l = points.length;
    points.resize(l + 1);
    points[l] = p;
    return points;
   ...
   private Slot[] append_slot(Slot[] slots, Slot s) {
    int l = slots.length;
    slots.resize(l + 1);
    slots[l] = s;
    return slots;
   ...
   private Step[] append_step(Step[] steps, Step s) {
    int l = steps.length;
    steps.resize(l + 1);
    steps[l] = s;
    stdout.printf("Net: steps appended: new len: %d\n", steps.length);
    return steps;
   }
 */
}
